...
In this demonstration, we will add upon the [previous](https://github.com/AnHosu/iot_poc/blob/master/publishing.md "simple publishing"), by having our gateway device, the Raspberry Pi, subscribe to messages on a topic.<br><br>
...
Like publishing, subscribing can be done with just one line of code. We just need to call the [.subscribe()](https://s3.amazonaws.com/aws-iot-device-sdk-python-docs/sphinx/html/index.html#AWSIoTPythonSDK.MQTTLib.AWSIoTMQTTClient.subscribe "subscribe docs") function of the MQTT client.
```python
AWSIoTMQTTClient.subscribe(topic, 1, callback_function)
```
...
```
In the next section, we will use this function to do cool things.<br><br>
